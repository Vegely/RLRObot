...
\section{Fundamentos matemáticos}
		Los apartados de procesos de decisión de Markov, aprendizaje por diferencias temporales y métodos de gradiente de política se basan en los capítulos 3, 6 y 13 de \cite{barto2021reinforcement}, respectivamente. El subapartado sobre soft actor-critic se apoya en \cite{haarnoja2018soft}.
		...
		\subsubsection{Aprendizajes episódico y continuo}
		...
		\subsubsection{Políticas y funciones de valor}
		...
		Los métodos de control expuestos en la sección anterior se centran en aproximar la función acción-valor. Este tipo de enfoques se encuadra en lo que se conoce como métodos acción-valor: aprenden los valores de las acciones y selecciona una acción basándose en los valores aprendidos. La política se deriva implícitamente de la maximización $\arg\max_a Q(s,a)$. \\
		
		Por otro lado, los métodos anteriores son tabulares: guardan registros con el valor de cada pareja acción-estado y los van actualizando. A medida que los espacios de estados y acciones crecen, los requisitos de memoria y tiempo para pasar por todos los estados se vuelven inasumibles. Los métodos aproximados buscan sustituir estas tablas por funciones matemáticas parametrizadas (p.ej. una red neuronal). \\
		
		Los métodos de gradiente de política aprenden directamente los parámetros de una política, por lo que no necesitan consultar las funciones de valor para seleccionar una acción. Esto no significa que las funciones de valor no sean necesarias para aprender la política. El objetivo de los métodos de gradiente de política es obtener los parámetros que maximicen una medida de rendimiento $J(\theta)$:
		
		\begin{align}
		& \pi(a \mid s ; \theta) = \Pr\{A_t = a \mid S_t = s; \theta_t = \theta \} \\
		& \theta_{t+1} = \theta_t + \alpha \widehat{\nabla_\theta J(\theta_t)} && (\text{Ascenso del gradiente})
		\end{align}
		
		Los métodos que, además, aprenden una aproximación de las funcinoes de valor se denominan actor-critic. El actor se relaciona con la política aprendida, mientras que el crítico se refiere a la función de valor. \\
		
		Los métodos de gradiente de política presentan algunas ventajas frente a los métodos acción-valor:
		
		\begin{itemize}
			\item Permiten aproximar políticas deterministas sin depender de parámetros de temperatura y exploración que son difíciles de ajustar.
			\item Permiten aproximar políticas estocásticas, superando a los métodos acción-valor que tienden a elegir $\arg\max_aQ(s,a)$.
			\item El cálculo de la política a veces puede ser más simple que el de la función acción-valor, que depende de la recompensa futura que se espera obtener.
			\item Se puede introducir conocimiento previo en la parametrización de la política.
			\item Las probabilidades de acción cambian de forma suave debido a que las políticas son continuas. En los métodos acción-valor se pueden dar saltos bruscos para cambios pequeños en los valores de las acciones debido a la función máximo.
		\end{itemize}
		
		Para el caso episódico ($\gamma=1$), la medida de rendimiento es lo bien que está posicionado el algoritmo al inicio de cada episodio:
		
		\begin{equation}
		J(\theta) = v_{\pi_\theta}(s_0)
		\end{equation}
		
		Donde $v_{\pi_\theta}$ es la función estado-valor real para la política $\pi_\theta$. El gradiente del rendimiento se apoya en el teorema del gradiente de política:
		
		\begin{align}
		\nabla v_\pi(s) &= \nabla \left[ \sum_a \pi(a|s)q_\pi(s, a) \right] , \quad \forall s \in \mathcal{S} && \text{(Ley de la probabilidad total)} \nonumber \\
		&= \sum_a \left[ \nabla \pi(a|s) q_\pi(s,a) + \pi(a|s) \nabla q_\pi(s,a) \right] && \text{(Derivada del producto)} \nonumber \\
		&= \sum_a \left[ \nabla \pi(a|s) q_\pi(s,a) + \pi(a|s) \nabla \sum_{s',r} p(s', r|s, a)(r + v_\pi(s')) \right] && \text{(Ecuación de Bellman)} \nonumber \\
		&= \sum_a \left[ \nabla \pi(a|s) q_\pi(s,a) + \pi(a|s) \sum_{s'} p(s'|s, a) \nabla v_\pi(s') \right] && \text{(Probabilidad marginal de $s'$)} \nonumber \\
		&= \phi(s) + \sum_{s'} \underbrace{\sum_a \pi(a|s) p(s'|s, a)}_{\Pr(s \to s', 1, \pi)} \nabla v_\pi(s') && \left( \phi(s) = \sum_a \nabla \pi(a|s) q_\pi(s,a) \right) \nonumber \\
		&= \phi(s) + \sum_{s'} \text{Pr}(s \to s', 1, \pi) \left[ \phi(s') + \sum_{s''} \text{Pr}(s' \to s'', 1, \pi) \nabla v_\pi(s'') \right] \nonumber \\
		&= \phi(s) + \sum_{s'} \text{Pr}(s \to s', 1, \pi) \phi(s') \nonumber \\
		&\qquad + \sum_{s''} \underbrace{\left( \sum_{s'} \text{Pr}(s \to s', 1, \pi) \text{Pr}(s' \to s'', 1, \pi) \right)}_{\Pr(s \to s'', 2, \pi)} \nabla v_\pi(s'') \nonumber \\
		&=\sum_{x \in \mathcal{S}} \sum_{k=0}^\infty \Pr(s \to x, k, \pi) \phi(x)
		\end{align}
		
		Aplicado al rendimiento para el caso episódico:
		
		\begin{align}
		\nabla J(\theta) &= \nabla v_\pi(s_0) \nonumber \\
		&= \sum_s \underbrace{\sum_{k=0}^\infty \text{Pr}(s_0 \to s, k, \pi)}_{\eta(s)} \sum_a \nabla \pi(a|s) q_\pi(s, a) \nonumber \\
		&= \sum_{s'} \eta(s') \sum_s \underbrace{\frac{\eta(s)}{\sum_{s'} \eta(s')}}_{\mu(s)} \sum_a \nabla \pi(a|s) q_\pi(s, a) && \text{(Normalización)} \nonumber \\
		&\propto \sum_s \mu(s) \sum_a \nabla \pi(a|s) q_\pi(s, a) && \left(\sum_{s'} \eta(s') = \text{cte} \right)
		\end{align}
		
		\subsubsection{REINFORCE}
		El sumatorio del teorema del gradiente de política puede ser inviable de calcular analíticamente. Una solución pasa por realizar una aproximación de Monte-Carlo mediante la esperanza sobre un conjunto de muestras. Dado que $\mu(s)$ representa la proporción de veces que se visita cada estado bajo la política $\pi$ (i.e. la probabilidad de cada estado):
		...
		Es decir, obteniendo muestras de $\sum_a q_\pi(S_t, a) \nabla_\theta \pi(a \mid S_t ; \theta)$ se podría aproximar la esperanza anterior. Dado que el espacio de acciones puede tener una elevada dimensión, se aproxima el sumatorio por otra esperanza (nótese que es necesario conocer la probabilidad con la que ocurre cada acción, $\pi(a \mid S_t ; \theta)$:
		
		\begin{align}
		\nabla_\theta J(\theta) &\propto \mathbb{E}_{\pi}\left[ \pi(a \mid S_t ; \theta) \sum_a q_\pi(S_t, a) \frac{\nabla_\theta \pi(a \mid S_t ; \theta)}{\pi(a \mid S_t ; \theta)} \right] \nonumber \\
		&= \mathbb{E}_{S_t \sim \mu}\left[ \mathbb{E}_{A_t \sim \pi} \left[q_\pi(S_t, A_t) \nabla_\theta \log \pi(A_t \mid S_t; \theta) \mid S_t \right] \right] && (\nabla \log(x) = \frac{\nabla x}{x}) \nonumber \\
		&= \mathbb{E}_{S_t \sim \mu, A_t \sim \pi}\left[ q_\pi(S_t, A_t) \nabla_\theta \log \pi(A_t \mid S_t; \theta) \right]  && (\text{Ley de la esperanza total}) \nonumber \\
		&= \mathbb{E}_{S_t, A_t} \left[ \mathbb{E}_\pi [ G_t \mid S_t, A_t ] \nabla_\theta \log \pi(A_t | S_t; \theta) \right] && \left(q_\pi(S_t, A_t) = \mathbb{E}_\pi[G_t \mid S_t, A_t]) \right) \nonumber \\
		&= \mathbb{E}_{S_t, A_t} \left[ \mathbb{E}_\pi [ G_t \nabla_\theta \log \pi(A_t | S_t; \theta) \mid S_t, A_t ] \right] \nonumber \\
		&= \mathbb{E}_{S_t, A_t} [ G_t \nabla_\theta \log \pi(A_t | S_t; \theta) ] && (\text{Ley de la esperanza total})
		\end{align}
		
		Aplicando el método de Monte-Carlo sobre la expresión final obtenida se aproxima la esperanza por la muestra de la iteración actual, dando lugar al esquema de actualización por ascenso del gradiente:
		
		\begin{equation}
		\theta_{t+1} = \theta_t + \alpha G_t \nabla_\theta \log(\pi(A_t \mid S_t ; \theta_t))
		\end{equation}
		
		El problema de la ecuación (algoritmo REINFORCE) anterior es que la varianza puede tomar valores altos si la recompensa varía mucho. Una solución es restar una referencia en el gradiente del rendimiento:
		
		\begin{align}
		&\begin{aligned}
		\nabla_\theta J(\theta) &\propto \sum_s \mu(s) \sum_a \left( q_\pi(s, a) - b(s) \right) \nabla_\theta \pi(a \mid s; \theta) \\
		&= \mathbb{E}_{\pi}\left[ \sum_a \left( q_\pi(S_t, a) - b(S_t) \right) \nabla_\theta \pi(a \mid S_t ; \theta) \right]
		\end{aligned} \\
		&\mathbb{E}_{\pi}\left[ \sum_a b(S_t) \nabla_\theta \pi(a \mid S_t ; \theta) \right] = \mathbb{E}_{\pi}\left[ b(S_t) \nabla_\theta \sum_a \pi(a \mid S_t ; \theta) \right] = \mathbb{E}_{\pi}\left[ b(S_t) \nabla_\theta 1 \right] = 0 \\
		&V(\nabla_\theta J(\theta)) = \mathbb{E}_\pi\left[ \left( (G_t - b(S_t)) \nabla_\theta \log \pi(A_t \mid S_t; \theta) \right)^2 \right] - \left( \nabla_\theta J(\theta) \right)^2
		\end{align}
		
		El algoritmo anterior se conoce como REINFORCE con línea de base. Cuando $b(S_t) = 0$ se tiene el REINFORCE clásico. Atendiendo a la varianza, solamente el primer término depende de la línea de referencia. Una línea de base conveniente podría ser una aproximación de la función de valor $\hat{v}(S_t, w)$, donde $w$ son los parámetros de la función que la aproxima. El esquema de aprendizaje de este método se podría resumir en:
		...
		El algoritmo REINFORCE depende del valor de $G_t$ calculado al final del episodio. Por lo tanto, puede pasar mucho tiempo hasta que se produzca una actualización de los parámetros. Siguiendo la filosofía del aprendizaje por diferencias temporales, se podría mirar la aproximación de la función estado-valor antes y después de ejecutar la acción para realizar una actualización a partir de una estimación de la recompensa $G_t$:
		
		\begin{align}
		\theta_{t+1} &= \theta_t + \alpha \left( R_{t+1} + \gamma \hat{v}(S_{t+1}, w) - \hat{v}(S_t, w) \nabla_\theta \log \pi(A_t, \mid S_t ; \theta_t) \right) \nonumber \\ 
		&= \theta_t + \alpha \delta_t \nabla_\theta \log \pi(A_t, \mid S_t ; \theta_t)
		\end{align}
		
		\subsubsection{Soft actor-critic}
		El algoritmo soft actor-critic es un método off-policy que busca maximizar la recompensa junto con la entropía de la política. De forma genérica:
		
		\begin{equation}
		J(\theta) = \mathbb{E}_{S_t, A_t} \left[ \sum_{t=0}^{\infty} \gamma^t \left( R_{t+1} - \alpha \log \pi(A_t \mid S_t ; \theta) \right) \right]
		\end{equation}
		
		Donde $\alpha$ es un parámetro de temperatura que regula la importancia del término de entropía. Esta formulación presenta varias ventajas:
		
		\begin{itemize}
			\item La exploración del entorno es más amplia, dejando de lado caminos muy complejos.
			\item La política captura múltiples maneras de hacer las tareas cercanas a la óptima. Si hubiera varias acciones factibles para resolver una situación, todas tenderían a tener la misma probabilidad.
		\end{itemize}
		
		El problema anterior se puede resolver para problemas tabulares mediante el algoritmo de iteración de política suave, consistente en repetir los siguientes pasos hasta la convergencia a la política óptima\cite{haarnoja2018soft}:
		
		\begin{enumerate}
			\item Evaluación suave de la política:
			
			\begin{align}
			& Q^{k+1}(s, a) = \mathcal{T}^\pi Q^k(s, a) \\
			& \begin{aligned}
			\mathcal{T}^\pi Q(s, a) &= \mathbb{E}_\pi \left[ R_{t+1} + \gamma V(S_{t+1}) \mid S_t = s, A_t = a \right] && (\text{Operador de Bellman}) \\
			&= \mathbb{E}_\pi \left[ R_{t+1} \mid S_t = s, A_t = a \right] + \gamma \mathbb{E}_\pi \left[ V(S_{t+1}) \mid S_t = s, A_t = a \right] \\
			&= r(s,a) + \gamma \mathbb{E}_\pi \left[ V(S_{t+1}) \mid S_t = s, A_t = a \right]
			\end{aligned} \\
			& V(s) = \mathbb{E}_{A_t} \left[ Q(S_t, A_t) - \alpha \log \pi(A_t \mid S_t) \mid S_t = s \right] \label{eq:estado_valor_obj}
			\end{align}
			
			\item Mejora suave de la política: estima la nueva política minimizando la divergencia de Kullback-Leibler con $\frac{\exp(Q_{\pi_t}(S_t, \cdot))}{Z_{\pi_t}(S_t)}$:
			
			\begin{equation}
			\pi_{t+1} = \arg\min_{\pi' \in \Pi} D_{KL} \left( \pi'(\cdot \mid S_t) \middle\| \frac{\exp(Q_{\pi_t}(S_t, \cdot))}{Z_{\pi_t}(S_t)} \right)
			\end{equation}
			
			Esto asegura que $Q_{\pi_{t}}(s, a) \ge Q_{\pi_{t}}(s, a) \quad \forall s, a \in \mathcal{S} \times \mathcal{A}$.
			
		\end{enumerate}
		
		Los espacios de acciones y estados continuos y grandes requieren aproximar la iteración de política suave. Es decir, se usan redes neuronales para estimar por descenso del gradiente los parámetros de:
		
		\begin{itemize}
			\item Función estado-valor: podría obtenerse a partir de la ecuación \ref{eq:estado_valor_obj}, pero es preferible entrenar una red por motivos de estabilidad. La idea es minimizar la diferencia entre este valor teórico y el aproximado por la red de parámetros $\psi$:
			
			\begin{align}
			& J_V(\psi) = \mathbb{E}_{S_t \sim \mathcal{D}} \left[ \frac{1}{2} \left( V_\psi(S_t) - \mathbb{E}_{A_t \sim \pi_\phi} \left[ Q_\theta(S_t, A_t) - \alpha \log \pi_\phi(A_t \mid S_t) \right] \right)^2 \right] \\
			& \nabla_\psi J_V(\psi) = \mathbb{E}_{S_t \sim \mathcal{D}} \left[ \nabla_\psi V_\psi(S_t) \left( V_\psi(S_t) - Q_\theta(S_t, A_t) + \alpha \log \pi_\phi(A_t \mid S_t) \right) \right]
			\end{align}
			
			Donde $\mathcal{D}$ es un buffer de repetición característico de los algoritmos off-policy que almacena valores ya observados. En cada iteración, aproxima la esperanza sobre las acciones por el valor de la muestra según la política actual. Además, se puede calcular la esperanza sobre las muestras de políticas pasadas debido a que la política de comportamiento es diferente a la política objetivo.
			
			\item Función acción-valor: buscan imitar el operador de Bellman aplicado sobre la función acción valor, minimizando el error a través de la actualización de los parámetros $\theta$ de la red:
			
			\begin{align}
			& J_Q(\theta) = \mathbb{E}_{(S_t, A_t) \sim \mathcal{D}} \left[ \frac{1}{2} \left( Q_\theta(S_t, A_t) - \left( R_{t+1} + \gamma \mathbb{E}_{S_{t+1}} \left[ V_{\bar{\psi}}(S_{t+1}) \right] \right) \right)^2 \right] \\
			& \nabla_\theta J_Q(\theta) = \mathbb{E}_{(S_t, A_t) \sim \mathcal{D}} \left[ \nabla_\theta Q_\theta(S_t, A_t) \left( Q_\theta(S_t, A_t) - R_{t+1} - \gamma V_{\bar{\psi}}(S_{t+1}) \right) \right]
			\end{align}
			
			Donde $\bar{\psi} = \tau \psi + (1 - \tau)\bar{\psi}$ es una media móvil sobre los parámetros de la red de la función acción-valor. Utilizar los pesos $\bar{\psi}$ en este punto ayuda a estabilizar el entrenamiento. Para evitar que la red sea demasiado optimistas, se entrenan dos redes Q de forma independiente y se toma el mínimo entre ambas para actualizar tanto la política objetivo como la función estado-valor.
			
			\item Política: miniiza la divergencia de Kullback-Leibler entre la distribución $\frac{\exp(Q_\theta(S_t, \cdot))}{Z_\theta(S_t)}$ deseada de la política y la aproximada por la red mediante los parámetros $\phi$:
			...
			& \begin{aligned}
			\nabla_\phi J_\pi(\phi) &= \mathbb{E}_{S_t \sim \mathcal{D}, \epsilon_t \sim \mathcal{N}} \bigg[ \alpha \nabla_\phi \log \pi_\phi(A_t \mid S_t) \\
			&\qquad + \left( \alpha \nabla_{A_t} \log \pi_\phi(A_t \mid S_t) - \nabla_{A_t} Q_\theta(S_t, A_t) \right) \nabla_\phi f_\phi(\epsilon_t; S_t) \bigg]
			\end{aligned}
			\end{align}
			
			Para derivar la política con baja varianza, se emplea el truco de reparametrización: la acción se calcula como una transformación determinista de un ruido externo $\epsilon_t$ mediante la red $f_\phi$:
